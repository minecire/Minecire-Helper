...
{
	[CustomEntity("MinecireHelper/SingleSpike = GenerateSingleSpike")]
	public class SingleSpike : Entity
    {
		public static Entity GenerateSingleSpike(Level level, LevelData levelData, Vector2 offset, EntityData entityData)
        {
			return new SingleSpike(entityData.Position + offset, entityData.Attr("direction"), entityData.Attr("type"));
        }

		private Spikes.Directions getDirectionFromString(String dir)
        {
			if (dir == "Up")
				return Spikes.Directions.Up;
			if (dir == "Left")
				return Spikes.Directions.Left;
			if (dir == "Right")
				return Spikes.Directions.Right;

			return Spikes.Directions.Down;
		}
        public SingleSpike(Vector2 position, String direction, String type) : base(position)
		{
			base.Depth = -1;
			...
			case Spikes.Directions.Up:
				base.Collider = new Hitbox((float)size, 3f, 0f, 5f);
			...
			case Spikes.Directions.Left:
				base.Collider = new Hitbox(3f, (float)size, 5f, 0f);
				base.Add(new LedgeBlocker(null));
				break;
		...
		public override void Added(Scene scene)
		{
			...
			{
				List<MTexture> atlasSubtextures = GFX.Game.GetAtlasSubtextures("MinecireHelper/danger/single_spike/" + this.spikeType + "_" + str);
				Image image = new Image(Calc.Random.Choose(atlasSubtextures));
				switch (this.Direction)
				{
					case Spikes.Directions.Up:
						image.JustifyOrigin(0.5f, 0.15f);
						image.Position = Vector2.UnitX * 2f + Vector2.UnitY;
						break;
					case Spikes.Directions.Down:
						...
						break;
					case Spikes.Directions.Left:
						image.JustifyOrigin(0.15f, 0.5f);
						image.Position = Vector2.UnitY * 2f + Vector2.UnitX;
						break;
					case Spikes.Directions.Right:
		...
		private void AddTentacle(float i)
		{
			...
			sprite.Position = ((this.Direction == Spikes.Directions.Up || this.Direction == Spikes.Directions.Down) ? Vector2.UnitX : Vector2.UnitY) * (i + 0.5f) * 8f;
